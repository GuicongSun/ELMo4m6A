...
class map():
    ...
    def analyze_train(temp, OutputDir):
        trainning_result, validation_result = temp

        file = open(OutputDir + '/performance_train.txt', 'w')
    ...
    def analyze_test(temp, OutputDir):
        trainning_result = temp

        file = open(OutputDir + '/performance_test.txt', 'w')

        for x in [trainning_result]:

            file.write('testing_' + 'results\n')
        ...
        for x in [trainning_result]:
            ...
            # dataframe = pd.DataFrame({'mean_fpr': mean_fpr, 'mean_tpr': mean_tpr})
            # dataframe.to_csv(OutputDir + "/auc_testing.csv", index=False, sep=',')
